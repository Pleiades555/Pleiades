/* Pleiades shared authentication framework
   ...
   Purpose: site-wide username login, brand/page permissions, and optional GitHub users.json push.
   Security note: this is a static-site visibility/access helper. For true server-side protection, place the site behind Cloudflare Access or similar.
*/
(function () {
  'use strict';
  ...
  function getCurrentUsername() {
    return normaliseUsername(localStorage.getItem(CONFIG.currentUserKey));
  }

  function setCurrentUsername(username) {
    localStorage.setItem(CONFIG.currentUserKey, normaliseUsername(username));
  }
  ...
  function ensureShell() {
    ...
    shell.innerHTML = `
      <div id="pleiadesAccessDenied" class="pleiades-access-denied pleiades-hidden">
        <strong>Access required.</strong><br>
        Sign in with an approved Pleiades username to view this page.
      </div>
      ...
      <section id="pleiadesLoginPanel" class="pleiades-login-panel pleiades-hidden" aria-label="Pleiades login">
        <h2>Pleiades Access</h2>
        <p>Enter your approved username.</p>
        <label for="pleiadesUsernameInput">Username</label>
        <input id="pleiadesUsernameInput" type="text" autocomplete="username" placeholder="ash">
        <div class="pleiades-button-row">
          <button id="pleiadesDoLogin" type="button">Load Access</button>
          <button id="pleiadesCloseLogin" class="secondary" type="button">Close</button>
        </div>
      ...
      <aside id="pleiadesUserCard" class="pleiades-user-card pleiades-hidden" aria-label="Current user">
        <h3 id="pleiadesCardName">User</h3>
        <div id="pleiadesCardRole" class="role-line">Role</div>
        ...
        <div class="pleiades-button-row">
          <button id="pleiadesSwitchUser" class="secondary" type="button">Switch</button>
          <button id="pleiadesLogout" class="danger" type="button">Logout</button>
          <button id="pleiadesAdminOpen" data-access="site-admin" type="button">Admin</button>
        </div>
      </aside>
      ...
      <section id="pleiadesAdminPanel" class="pleiades-admin-panel pleiades-hidden" aria-label="Pleiades admin">
        ...
        <p>Create or update users, then optionally push <code>accounts/users.json</code> to GitHub using a fine-grained token.</p>
        <div class="pleiades-admin-grid">
          <label>Username<input id="adminUsername" type="text" placeholder="dealer123"></label>
          <label>Name<input id="adminName" type="text" placeholder="John Smith"></label>
          <label>Title<input id="adminTitle" type="text" placeholder="Parts Interpreter"></label>
          <label>Dealership<input id="adminDealer" type="text" placeholder="11290 - Metro Ford"></label>
          <label>Department<input id="adminDepartment" type="text" placeholder="Parts"></label>
          <label>Ford start date<input id="adminFordStart" type="date"></label>
        </div>
        <div id="pleiadesPermissionEditor" class="pleiades-permission-editor"></div>
        <div class="pleiades-button-row">
          <button id="adminSaveLocal" type="button">Save Locally</button>
          <button id="adminPushGithub" type="button">Push Users to GitHub</button>
          <button id="adminClose" class="secondary" type="button">Close</button>
        </div>
        ...
        <div id="pleiadesAdminStatus" class="pleiades-status"></div>
        <div id="pleiadesUserList" class="pleiades-user-list"></div>
      </section>
    `;
    ...
    document.getElementById('pleiadesUsernameInput').addEventListener('keydown', event => {
  ...
  function showLoginPanel() {
    document.getElementById('pleiadesLoginPanel').classList.remove('pleiades-hidden');
    setTimeout(() => document.getElementById('pleiadesUsernameInput').focus(), 0);
  }

  function hideLoginPanel() {
    document.getElementById('pleiadesLoginPanel').classList.add('pleiades-hidden');
  }

  function handleLogin() {
    const input = document.getElementById('pleiadesUsernameInput');
    const result = login(input.value);
    setLoginStatus(result.message, result.ok);
    if (result.ok) hideLoginPanel();
  }

  function setLoginStatus(message, ok) {
    const status = document.getElementById('pleiadesLoginStatus');
    if (!status) return;
    status.textContent = message;
    status.className = 'pleiades-status ' + (ok ? 'good' : 'bad');
  }

  function setAdminStatus(message, ok) {
    const status = document.getElementById('pleiadesAdminStatus');
    if (!status) return;
    status.textContent = message;
    status.className = 'pleiades-status ' + (ok ? 'good' : 'bad');
  }
  ...
  function updateUserCard(user) {
    ...
    if (!user) {
      card.classList.add('pleiades-hidden');
      loginButton.classList.remove('pleiades-hidden');
      return;
    }

    card.classList.remove('pleiades-hidden');
    loginButton.classList.add('pleiades-hidden');
  ...
  function openAdminPanel() {
    const user = getCurrentUser();
    if (!hasAccess('site-admin', user)) {
      setLoginStatus('Admin permission required.', false);
      return;
    }
  ...
  function buildPermissionEditor(selected) {
    ...
    const groups = state.permissions || {};
    host.innerHTML = Object.keys(groups).map(groupKey => {
      const items = groups[groupKey] || [];
      return `<fieldset><legend>${escapeHtml(groupKey)}</legend>${items.map(item => {
  ...
  function getAdminFormUser() {
    ...
    return {
      username,
      name: document.getElementById('adminName').value.trim() || username,
      title: document.getElementById('adminTitle').value.trim() || 'Not specified',
      dealer: document.getElementById('adminDealer').value.trim() || 'Not specified',
      department: document.getElementById('adminDepartment').value.trim() || '',
      fordStartDate: document.getElementById('adminFordStart').value || '',
      role: permissions.includes('*') ? 'unrestricted' : 'custom',
      enabled: true,
      permissions,
  ...
  function saveAdminUserLocal() {
    try {
      ...
      if (index >= 0) {
        state.users[index] = { ...state.users[index], ...formUser, createdAt: state.users[index].createdAt || new Date().toISOString() };
      } else {
        state.users.push({ ...formUser, createdAt: new Date().toISOString() });
      }
      state.users = cleanUsers(state.users);
      renderUserList();
      setAdminStatus('Saved locally. Push to GitHub to update users.json.', true);
    } catch (error) {
      setAdminStatus(error.message, false);
    }
  }
  ...
  function renderUserList() {
    ...
    host.querySelectorAll('[data-edit-user]').forEach(button => {
      button.addEventListener('click', () => loadUserIntoAdmin(button.getAttribute('data-edit-user')));
    });
  }
  ...
  async function pushUsersToGitHub() {
    ...
    if (!token) {
      setAdminStatus('Paste a fine-grained GitHub token first.', false);
      return;
    }
    ...
    try {
      ...
      const getResponse = await fetch(`${apiUrl}?ref=${encodeURIComponent(CONFIG.githubBranch)}`, {
      ...
      const putResponse = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          message: 'Update Pleiades user accounts',

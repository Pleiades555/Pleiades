...
def validate_vehicles(data: dict) -> None:
    ...
    for i, row in enumerate(rows, 1):
        ...
        for prefix in row.get("vinPrefixes", []):
            ...
            elif not re.fullmatch(r"[A-HJ-NPR-Z0-9]{8,17}", prefix):
                ERRORS.append(f"{label}: invalid VIN-prefix characters {prefix!r}")
            if prefix in prefixes and prefixes[prefix] != rid:
                ERRORS.append(f"{label}: VIN prefix {prefix} also belongs to {prefixes[prefix]}")
            prefixes[prefix] = rid

        for vin in row.get("exampleVins", []):
            if not isinstance(vin, str) or not re.fullmatch(r"[A-HJ-NPR-Z0-9]{17}", vin):
                ERRORS.append(f"{label}: invalid exact/example VIN {vin!r}")
                continue
            if vin in exact_vins and exact_vins[vin] != rid:
                ERRORS.append(f"{label}: exact VIN {vin} also belongs to {exact_vins[vin]}")
            exact_vins[vin] = rid
...
def validate_identifiers(data: dict, known_brands: set[str]) -> None:
    ...
    values: set[str] = set()
    allowed_types = {
        "australian-surrogate-vin",
        "new-zealand-surrogate-vin",
